...
const {
  ...
  energyTodayKwh,
  climateZones,
} = useDashboardData(status, states, semanticHome);
...
const discoveryLabel = computed(() => {
  if (status.value === "connected") {
    return `${semanticHome.value.areas.length} areas discovered`;
  }
    ...
    <main>
      <header class="topbar">
        ...
        <div class="top-status">
          <button
            class="pill connect-pill"
            type="button"
            @click="showConnectionModal = true"
          >
            <component
              :is="status === 'connected' ? Wifi : WifiOff"
              :size="14"
            />
            {{ discoveryLabel }}
            <Settings2 :size="13" />
      ...
      <section class="ai-strip">
        ...
        <div>
          <strong>House looks calm.</strong>
          <p>
            AI-ready summary surface. Connect an agent to replace this with
            contextual insight.
          </p>
        </div>
      ...
      <section class="dashboard-grid">
        ...
        <EnergyCard
          :watts="powerWatts ?? 1420"
          :today-kwh="energyTodayKwh ?? 9.8"
        />

        <ClimateCard
          v-if="climateZones.length"
          :zones="climateZones"
        />
        ...
        <ApplianceCard
          ...
          state="38 min remaining"
          detail="Binding-ready appliance surface"
          :progress="64"
        />
        ...
        <ApplianceCard
          ...
          title="Robot vacuum"
          state="Docked"
          detail="Binding-ready vacuum surface"
        />
